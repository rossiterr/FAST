...
def analyze_shingles_coverage(coverage_file, shingles_file, output_prefix, entity_name='branch'):
    """
    ...
    plt.title(f'Comparação: {entity_label} vs Hashed Shingles Coverage\n({title_name})', 
    ...
    with open(output_file, 'w') as f:
        ...
        f.write(f'Percentage\tTests_Executed\t{entity_col_name}\tShingles_Coverage\t'
                'Unique_Shingles_Ratio\n')
    ...
    print(f"Taxa de compressão ({entity_name}/shingles): {compression_ratio:.2f}x")
    print(f"Média de shingles por teste: {np.mean([len(s) for s in shingles]):.1f}")
    
    # Correlação entre as métricas
    print(f"\nCorrelação ({entity_label} vs Shingles Coverage): {correlation:.4f}")
    
    if correlation > 0.9:
        print(f"✓ Alta correlação: Shingles coverage é excelente proxy para {entity_name} coverage")
    elif correlation > 0.7:
        print(f"~ Correlação boa: Shingles coverage captura bem a cobertura")
    elif correlation > 0.5:
        print(f"⚠ Correlação moderada: Shingles coverage captura parcialmente a cobertura")
    else:
        print(f"✗ Baixa correlação: Shingles coverage pode não refletir bem a cobertura")
...
if __name__ == "__main__":
    ...
    print("="*60)
    print("ANÁLISE DE COBERTURA: Coverage vs Hashed Shingles")
    ...
    for project in projects:
        ...
        for entity in entities:
            ...
            # Definir caminho de saída: correlation_hashed_shingles/{entity}_cov/{project}/{project}_bbox
            output_prefix = os.path.join('correlation_hashed_shingles', f'{entity}_cov', project, f'{project}_bbox')
            
            try:
                analyze_shingles_coverage(coverage_file, shingles_file, output_prefix, entity_name=entity)
    ...
    print(f"Resultados salvos em: correlation_hashed_shingles/{{branch,function,line}}_cov/")
    print("="*60)
